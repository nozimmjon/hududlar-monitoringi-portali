...
OUT_HTML = ROOT / "platform prototypes" / "andijon_integrated_platform_v6.html"
...
<head>
  ...
  <title>Андижон вилояти мониторинг платформаси · v6</title>
  <style>
    :root {
      --bg: #eef4fa;
      --paper: #ffffff;
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 1180px) {
      ...
      .task-workspace, .task-filter, .task-summary-strip { grid-template-columns: 1fr; }
    ...
    @media (max-width: 760px) {
      ...
      .front-kpis, .workflow, .task-board, .cards-3, .command-summary, .context-strip, .district-controls, .link-grid, .district-kpis { grid-template-columns: 1fr; }
