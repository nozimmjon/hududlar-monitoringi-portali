...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .promise-step:last-child::after { display: none; }
    .promise-step span { display: block; color: var(--blue); font-size: 11px; font-weight: 950; text-transform: uppercase; }
    .promise-step strong { display: block; margin-top: 5px; font-size: 15px; line-height: 1.2; }
    .promise-step small { display: block; margin-top: 5px; color: var(--muted); font-size: 12px; line-height: 1.3; }
    ...
    .drawer-panel {
      ...
      background: #fff;
      border-left: 1px solid var(--line);
      box-shadow: -24px 0 50px rgba(7, 26, 45, .22);
      overflow: auto;
    }
    ...
    @media (max-width: 1180px) {
      ...
      .task-workspace, .task-filter, .task-summary-strip, .promise-chain { grid-template-columns: 1fr; }
      .promise-step::after { display: none; }
    ...
    @media (max-width: 760px) {
      ...
      .front-kpis, .workflow, .task-board, .cards-3, .command-summary, .context-strip, .district-controls, .link-grid, .district-kpis, .promise-chain { grid-template-columns: 1fr; }
...
<body>
    ...
    <div class="mast">
      <div class="brand">
        ...
        <div>
          ...
          <p>кафолат KPI · туманлар кесими · топшириқлар ижроси</p>
        </div>
      </div>
  ...
  <div class="shell">
    <aside class="sidebar">
      <div class="side-title">
        ...
        <span>Кафолат KPIдан туманлар ва топшириқлар ижросига ўтиш.</span>
      </div>
  ...
  <aside class="drawer" id="districtDrawer" aria-hidden="true">
    <div class="drawer-panel" role="dialog" aria-modal="true" aria-labelledby="drawerTitle">
      <div class="drawer-head">
        <div>
          <div class="eyebrow">Туман профили</div>
          <h3 id="drawerTitle">Туман профили</h3>
        </div>
        <button class="mini-button" id="drawerClose" type="button">Ёпиш</button>
      </div>
  ...
  <script>
    ...
    function promiseChain(selected) {
      const selectedTasks = tasksForKpi(selected.id);
      const openTasks = selectedTasks.filter(t => (t.status || "grey") !== "green").length;
      const hasDistricts = selected.id === "grp" || districtSelectorDefs().some(def => def.id === selected.id);
      const districtHint = selected.id === "grp"
        ? "ЯҲМ туман кесимида эмас, таркибий кўрсаткичлар орқали очилади."
        : hasDistricts ? "Туманлар кесимида мақсад/амалда/ижро очилади." : "Бу KPI учун туман кесими алоҳида берилмаган.";
      return `<div class="promise-chain">
        <div class="promise-step">
          <span>1. Кафолат KPI</span>
          <strong>${selected.short}</strong>
          <small>Кафолат хати ва Excel жадвалларида белгиланган режа/чораклик мақсад.</small>
        </div>
        <div class="promise-step">
          <span>2. Боғланган топшириқлар</span>
          <strong>${openTasks}/${selectedTasks.length}</strong>
          <small>Бажарилмаган / жами. Тўлиқ матн “Топшириқлар” экранида KPI бўйича гуруҳланади.</small>
        </div>
        <div class="promise-step">
          <span>3. Туманлар мониторинги</span>
          <strong>${hasDistricts ? "Мавжуд" : "Йўқ"}</strong>
          <small>${districtHint}</small>
        </div>
      </div>`;
    }
    ...
    function renderDashboard() {
      ...
      $$("[data-district]", $("#dashboardPage")).forEach(card => card.addEventListener("click", () => {
        ...
        render();
        setTimeout(() => openDistrictDrawer(state.district), 0);
      }));
    ...
    function districtDrawerHtml(d, kpi, cfg) {
      const row = districtKpi(d, kpi.id, cfg.primaryPeriod || state.period);
      const status = rowStatus(row);
      const summary = districtTaskSummary(d, kpi.id);
      const taskClass = summary.total && summary.unfinished ? "red" : summary.total ? "green" : "grey";
      return `
        ...
        <div class="profile-metrics" style="margin-top:12px">
        ...
        <div class="preview-task-summary">
          <strong>KPI топшириқлари: ${summary.unfinished}/${summary.total}</strong>
          <span>Бу туман учун танланган KPIга боғланган бажарилмаган / жами топшириқлар. Тўлиқ рўйхат “Топшириқлар” экранида KPI бўйича очилади.</span>
        </div>
        <div class="task-list" style="margin-top:12px">
          ${summary.tasks.slice(0, 4).map(compactTaskCard).join("") || `<p class="muted">Бу KPI бўйича топшириқ топилмади.</p>`}
        </div>
        <div class="action-row">
          <button class="mini-button primary" data-drawer-page="tasks">Топшириқларни очиш</button>
          <button class="mini-button" data-drawer-page="districts">Туманлар жадвали</button>
        </div>`;
    }

    function openDistrictDrawer(name) {
      const d = DATA.districts.find(item => item.name === name) || currentDistrict();
      state.district = d.name;
      const kpi = currentKpiDef();
      const cfg = districtTableConfig(kpi);
      $("#drawerTitle").textContent = `${d.name}: ${kpi.short}`;
      $("#drawerBody").innerHTML = districtDrawerHtml(d, kpi, cfg);
      $("#districtDrawer").classList.add("open");
      $("#districtDrawer").setAttribute("aria-hidden", "false");
      $$("[data-drawer-page]", $("#drawerBody")).forEach(btn => btn.addEventListener("click", () => {
        closeDistrictDrawer();
        state.page = btn.dataset.drawerPage;
        render();
      }));
    }

    function closeDistrictDrawer() {
      $("#districtDrawer").classList.remove("open");
    ...
    function renderDistrictsPage() {
      ...
      $$("[data-profile-district]", $("#districtsPage")).forEach(el => el.addEventListener("click", event => {
        ...
        openDistrictDrawer(state.district);
      }));
    ...
    function setPageMeta() {
      const meta = {
        dashboard: ["KPI", "Кафолат хатидаги асосий KPI, чораклик режа/амалда/ижро ва боғланган топшириқлар."],
        districts: ["Туманлар", `${currentKpiDef().short} бўйича 16 та туман/шаҳар кесими: мақсадли кўрсаткичлар ва ижро.`],
        tasks: ["Топшириқлар", "Кафолат хатидаги вазифалар танланган KPI билан боғланган ҳолда кўринади."]
      };
    ...
    function render() {
      ...
      if (state.page === "profile") state.page = "districts";
      if (state.page === "report") state.page = "tasks";
      ...
      $("#sectorFilter").classList.toggle("hidden", ["dashboard", "tasks", "districts"].includes(state.page));
      $("#searchBox").classList.toggle("hidden", ["dashboard", "districts"].includes(state.page));
    ...
    $("#drawerClose").addEventListener("click", closeDistrictDrawer);
    $("#districtDrawer").addEventListener("click", event => {
      if (event.target.id === "districtDrawer") closeDistrictDrawer();
    });
