...
OUT_HTML = ROOT / "platform prototypes" / "andijon_integrated_platform_v8.html"
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .exec-hero {
      ...
      border-radius: var(--r);
      background:
        linear-gradient(120deg, rgba(34, 100, 213, .12), rgba(255,255,255,0) 52%),
        #fff;
      box-shadow: var(--shadow);
      padding: 18px;
      min-width: 0;
    ...
    .exec-status-card span { display: block; color: var(--muted); font-size: 11px; font-weight: 950; }
    .exec-status-card strong { display: block; margin-top: 6px; font-size: 25px; line-height: 1; color: var(--ink); }
    .exec-status-card.green strong { color: var(--green); }
    .exec-status-card.amber strong { color: var(--amber); }
    .exec-status-card.red strong { color: var(--red); }
    .exec-status-card.grey strong { color: var(--grey); }
    ...
    .priority-row strong,
    .district-map-card strong { display: block; font-size: 13px; color: var(--ink); line-height: 1.2; }
    .priority-row span,
    .district-map-card span { display: block; margin-top: 4px; color: var(--muted); font-size: 11px; line-height: 1.25; }
    ...
    .district-map-card:hover,
    .district-map-card.active {
      border-color: var(--blue);
      background: #f5f9ff;
      transform: translateY(-1px);
    }

    .district-map-card.red { border-color: rgba(223, 64, 64, .35); }
    .district-map-card.amber { border-color: rgba(205, 141, 24, .35); }
    .district-map-card.green { border-color: rgba(18, 135, 88, .3); }
    .district-map-card.grey { border-color: rgba(124, 137, 151, .35); }
    ...
    .district-rank-item.active { border-color: var(--blue); background: var(--blue-soft); }
    .district-rank-item b { color: var(--muted); font-size: 12px; }
    .district-rank-item strong { display: block; font-size: 13px; }
    .district-rank-item span { display: block; margin-top: 2px; color: var(--muted); font-size: 11px; }

    .execution-audit-grid {
      display: grid;
    ...
    @media (max-width: 1180px) {
      ...
      .task-workspace, .task-filter, .task-filter.report-filter, .task-summary-strip, .workflow-strip, .kpi-route, .exec-dashboard, .quick-panels, .district-lens, .execution-audit-grid { grid-template-columns: 1fr; }
      ...
      .district-kpis, .district-map-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); }
    }
    ...
    @media (max-width: 760px) {
      ...
      .page-head h2, .exec-hero h3, .exec-hero p, .panel-head h3, .panel-head p, .priority-row strong, .priority-row span, .district-map-card strong, .district-map-card span { overflow-wrap: anywhere; }
      .exec-hero { padding: 16px; }
      .exec-hero h3 { font-size: 26px; }
      .panel-head { flex-wrap: wrap; }
      .priority-row { grid-template-columns: 1fr; }
      .chip { max-width: 100%; white-space: normal; line-height: 1.15; }
      .toolbar { justify-content: flex-start; }
      .dashboard-module-tabs, .module-heading, .front-kpis, .front-kpis.module-kpis.macro-layout, .workflow, .task-board, .cards-3, .command-summary, .context-strip, .district-controls, .link-grid, .district-kpis, .kpi-route, .exec-status-grid, .district-map-grid { grid-template-columns: 1fr; }
...
<body>
  ...
  <div class="shell">
    <aside class="sidebar">
      <div class="side-title">
        ...
        <span>KPI, туманлар ва киритилган ҳисоботларни кузатиш.</span>
      </div>
  ...
  <script>
    ...
    const periods = [
      { id: "q1", label: "I чорак", suffix: "I чорак" },
      { id: "h1", label: "II чорак", suffix: "I ярим йиллик" },
      { id: "m9", label: "III чорак", suffix: "9 ой" },
      { id: "year", label: "IV чорак / йиллик", suffix: "йил якуни" }
    ];
    ...
    function statusLabel(status) {
      return {
        green: "Яшил · бажарилган",
        amber: "Сариқ · эътибор талаб",
        red: "Қизил · кечиккан",
        grey: "Кулранг · маълумот йўқ"
      }[status] || "Кулранг · маълумот йўқ";
    }
    ...
    function dashboardModuleIntro(moduleId) {
      const text = {
        macro: "ЯҲМ ва унинг асосий таркибий кўрсаткичлари.",
    ...
    function statusOrder(status) {
      return { red: 0, amber: 1, grey: 2, green: 3 }[status] ?? 2;
    }

    function dashboardKpiSnapshot() {
      return kpiDefs.map(def => {
        const annual = annualPlanKpi(def.id);
        const q1 = dashboardPeriodKpi(def.id, "q1");
        const status = rowStatus(annual);
        return { def, annual, q1, status };
      });
    }

    function dashboardStatusCounts() {
      return dashboardKpiSnapshot().reduce((acc, item) => {
        acc[item.status] = (acc[item.status] || 0) + 1;
        return acc;
      }, { green: 0, amber: 0, red: 0, grey: 0 });
    }

    function priorityKpis(limit = 6) {
      return dashboardKpiSnapshot()
        .sort((a, b) => {
          const statusDiff = statusOrder(a.status) - statusOrder(b.status);
          if (statusDiff) return statusDiff;
          return a.def.short.localeCompare(b.def.short, "uz-Cyrl-UZ");
        })
        .slice(0, limit);
    }

    function districtRowsForKpiAll(kpiId) {
      ...
      const rows = DATA.districts.map(d => ({
        d,
        k: districtKpi(d, kpiId, cfg.primaryPeriod || state.period),
        task: districtTaskSummary(d, kpiId)
      }));
      return rows.sort((a, b) => {
        const ast = rowStatus(a.k), bst = rowStatus(b.k);
        const statusDiff = statusOrder(ast) - statusOrder(bst);
        if (statusDiff) return statusDiff;
        return districtAttentionValue(a, kpiId) - districtAttentionValue(b, kpiId);
      });
    }

    function executiveDistricts(kpiId = state.kpi, limit = 5) {
      if (!districtSelectorDefs().some(def => def.id === kpiId)) return [];
      return districtRowsForKpiAll(kpiId).slice(0, limit);
    }

    function executionSnapshot() {
      const reports = getExecutionReports();
      const waitingReports = reports.filter(r => ["submitted", "review"].includes(r.status));
      const rejectedReports = reports.filter(r => r.status === "rejected");
      const approvedReports = reports.filter(r => r.status === "approved");
      const openTasks = DATA.tasks.filter(t => (t.status || "grey") !== "green");
      return {
        reports,
        waitingReports,
        rejectedReports,
        approvedReports,
        openTasks,
        linkedTasks: DATA.tasks.filter(t => reports.some(report => report.taskId === t.id))
      };
    }

    function renderExecutiveOverview(selected) {
      const counts = dashboardStatusCounts();
      const priorities = priorityKpis();
      const districtItems = executiveDistricts(selected.id);
      const snapshot = executionSnapshot();
      const doneText = `${counts.green}/${kpiDefs.length}`;
      return `<section class="exec-dashboard">
        <div class="exec-hero">
          <div class="eyebrow">Андижон вилояти · 2026</div>
          <h3>KPI, туманлар ва ижро ҳисоботлари бир узвий занжирда</h3>
          <p>Биринчи экран раҳбар учун қисқа ҳолатни кўрсатади: асосий KPIлар, эътибор талаб қилаётган кўрсаткичлар, танланган KPI бўйича муаммоли туманлар ва тасдиқ кутayotgan ҳисоботлар.</p>
          <div class="exec-status-grid">
            <div class="exec-status-card green"><span>Яшил</span><strong>${counts.green}</strong></div>
            <div class="exec-status-card amber"><span>Сариқ</span><strong>${counts.amber}</strong></div>
            <div class="exec-status-card red"><span>Қизил</span><strong>${counts.red}</strong></div>
            <div class="exec-status-card grey"><span>Кулранг</span><strong>${counts.grey}</strong></div>
          </div>
        </div>
        <aside class="exec-priority">
          <div class="panel-head" style="margin:-14px -14px 0"><div><h3>Биринчи навбатда кўриш</h3><p>Рақам йўқ, тасдиқ кутмоқда ёки режага нисбатан хавф бор.</p></div><span class="chip blue">${doneText}</span></div>
          <div class="compact-list">
            ${priorities.map(item => `<button class="priority-row" type="button" data-kpi="${item.def.id}">
              <div><strong>${item.def.short}</strong><span>Йиллик: ${primaryMetric(item.annual)} · I чорак: ${primaryMetric(item.q1)}</span></div>
      ...
      <section class="quick-panels">
        <article class="panel">
          <div class="panel-head"><div><h3>Танланган KPI бўйича туманлар</h3><p>${selected.short}: эътибор талаб қиладиган ҳудудлар.</p></div><button class="mini-button primary" data-open-districts="${selected.id}">Туманлар</button></div>
          <div class="panel-body compact-list">
            ${districtItems.length ? districtItems.map(item => `<button class="priority-row" type="button" data-open-profile="${item.d.name}">
              <div><strong>${item.d.name}</strong><span>${districtMeasureNote(item.k, selected.id)}</span></div>
              <span class="chip ${rowStatus(item.k)}">${districtPrimaryValue(item.k, selected.id)}</span>
            </button>`).join("") : `<p class="muted">Бу KPI бўйича туман кесими йўқ.</p>`}
          </div>
        </article>
        <article class="panel">
          <div class="panel-head"><div><h3>Ижро ҳисоботлари</h3><p>Тасдиқланган ҳисоботларгина KPI “амалда” қийматига таъсир қилади.</p></div><button class="mini-button" data-open-execution>Журнал</button></div>
          <div class="panel-body">
            <div class="preview-metrics">
              <div class="preview-metric"><div><strong>Тасдиқ кутяпти</strong><small>киритилди / қайта кўришда</small></div><span class="chip amber">${snapshot.waitingReports.length}</span></div>
              <div class="preview-metric"><div><strong>KPIга ўтди</strong><small>тасдиқланган ҳисобот</small></div><span class="chip green">${snapshot.approvedReports.length}</span></div>
              <div class="preview-metric"><div><strong>Қайтарилди</strong><small>KPIга ўтмаган</small></div><span class="chip red">${snapshot.rejectedReports.length}</span></div>
            </div>
          </div>
        </article>
        <article class="panel">
          <div class="panel-head"><div><h3>Топшириқлар ҳолати</h3><p>Биринчи экранда номлар эмас, фақат назорат сигнали.</p></div><button class="mini-button" data-page-jump="tasks">Топшириқлар</button></div>
          <div class="panel-body">
            <div class="preview-metrics">
              <div class="preview-metric"><div><strong>Бажарилмаган</strong><small>жами T-топшириқлардан</small></div><span class="chip red">${snapshot.openTasks.length}/${DATA.tasks.length}</span></div>
              <div class="preview-metric"><div><strong>Ҳисобот боғланган</strong><small>ижро журнали орқали</small></div><span class="chip blue">${snapshot.linkedTasks.length}/${DATA.tasks.length}</span></div>
            </div>
          </div>
        </article>
      </section>`;
    ...
    function renderDashboard() {
      ...
      $("#dashboardPage").innerHTML = `
        ${renderExecutiveOverview(selected)}
      ...
      $$("button[data-kpi]:not(.front-kpi)", $("#dashboardPage")).forEach(btn => btn.addEventListener("click", () => {
        state.kpi = btn.dataset.kpi;
        state.dashboardModule = dashboardModuleForKpi(state.kpi);
        render();
      }));
      ...
      $$("[data-page-jump]", $("#dashboardPage")).forEach(btn => btn.addEventListener("click", () => {
      ...
      return `<div class="drivers">
        ...
        <div class="lagging">
          ...
          <div class="driver-grid">
            ...
            <div class="driver-card"><span>Янги омборлар режаси</span><strong>${fmt(warehouseTotals.newCount, 0)} та</strong><small>йиллик 33 та, 8 810 тонна</small></div>
          </div>
        </div>
      </div>`;
    }
    ...
    function districtAttentionValue(item, kpiId) {
      const value = districtRowValue(item, kpiId);
      return directionForKpi(kpiId) === "lower" ? -value : value;
    }
    ...
    function districtRowsForKpi(kpiId) {
      ...
      return rows.sort((a, b) => {
        ...
        const attentionDiff = districtAttentionValue(a, kpiId) - districtAttentionValue(b, kpiId);
    ...
    function renderDistrictMap(rows, kpi) {
      return `<div class="district-map-grid">
        ${rows.map(item => {
          const row = item.k;
          const status = rowStatus(row);
          const active = item.d.name === state.district ? "active" : "";
          return `<button class="district-map-card ${status} ${active}" type="button" data-select-district="${item.d.name}">
            <strong>${item.d.name}</strong>
            <span>${districtPrimaryValue(row, kpi.id)} · ${districtMeasureNote(row, kpi.id)}</span>
            <span class="chip ${status}">${statusLabel(status)}</span>
          </button>`;
        }).join("")}
      </div>`;
    ...
    function renderDistrictRanking(rows, kpi) {
      return `<div class="district-rank-list">
        ${rows.slice(0, 6).map((item, index) => {
          const status = rowStatus(item.k);
          const active = item.d.name === state.district ? "active" : "";
          return `<button class="district-rank-item ${active}" type="button" data-select-district="${item.d.name}">
            <b>${index + 1}</b>
            <div><strong>${item.d.name}</strong><span>${districtMeasureNote(item.k, kpi.id)}</span></div>
            <span class="chip ${status}">${districtPrimaryValue(item.k, kpi.id)}</span>
          </button>`;
        }).join("")}
      </div>`;
    }
    ...
    function renderDistrictsPage() {
      ...
      $("#districtsPage").innerHTML = `
        ...
        <div class="district-lens">
          <article class="panel">
            <div class="panel-head">
              <div><h3>Туманлар визуал кесими</h3><p>Ранглар танланган KPI бўйича ҳолатни кўрсатади. Карточкани босганда ўнгдаги профиль янгиланади.</p></div>
              <span class="chip blue">${periodLabel()}</span>
            </div>
            <div class="panel-body">${renderDistrictMap(ranked, kpi)}</div>
          </article>
        ...
        <div class="district-workspace">
          <article class="panel">
            <div class="panel-head">
              <div><h3>Батафсил жадвал</h3><p>${rangeText}</p></div>
              <span class="chip blue">солиштирув</span>
            </div>
            <div class="panel-body">${renderDistrictTable(ranked, kpi, cfg)}</div>
          </article>
          <article class="panel">
            <div class="panel-head"><div><h3>Эътибор талаб қиладиганлар</h3><p>Саралаш: қизил, сариқ, кулранг, кейин яшил.</p></div></div>
            <div class="panel-body">${renderDistrictRanking(ranked, kpi)}</div>
          </article>
        </div>`;
      ...
      $$("[data-select-district]", $("#districtsPage")).forEach(el => el.addEventListener("click", event => {
        if (el.tagName === "TR" && event.target.closest("button")) return;
    ...
    function renderExecutionPage() {
      ...
      const approvalQueue = allReports.filter(r => ["submitted", "review"].includes(r.status));
      $("#executionPage").innerHTML = `
        ...
        <div class="execution-audit-grid">
        <article class="panel">
          <div class="panel-head">
        ...
        <aside class="panel">
          <div class="panel-head">
            <div><h3>Тасдиқ навбати</h3><p>KPIга ҳали таъсир қилмаган ҳисоботлар.</p></div>
            <span class="chip amber">${approvalQueue.length}</span>
          </div>
          <div class="panel-body approval-queue">
            ${approvalQueue.length ? approvalQueue.slice(0, 8).map(report => `<article class="report-item">
              <header><strong>${h(report.kpiLabel || "KPI")}</strong><span class="chip ${reportStatusClass(report.status)}">${reportStatusLabel(report.status)}</span></header>
              <p>${h(report.district)} · ${h(report.periodLabel || "")} · ${h(report.actualValue || "—")} ${h(report.unit || "")}</p>
              <div class="action-row compact">
                <button class="mini-button" data-report-status="${report.id}:approved">Тасдиқлаш</button>
                <button class="mini-button danger" data-report-status="${report.id}:rejected">Қайтариш</button>
              </div>
            </article>`).join("") : `<p class="muted">Тасдиқ кутаётган ҳисобот йўқ.</p>`}
          </div>
        </aside>
        </div>`;
    ...
    function render() {
      ...
      const pages = ["dashboard", "tasks", "districts", "profile", "execution"];
      pages.forEach(page => {
